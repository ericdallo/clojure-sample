...
* [`src/clojure_sample/`](src/clojure_sample/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/clojure_sample/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `clojure-sample` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/clojure_sample/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clojure-sample.example
  (:require [clojure-sample.config :as config])
